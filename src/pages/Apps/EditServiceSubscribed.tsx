...
const EditServiceSubscribed = () => {
    ...
    useEffect(() => {
        dispatch(setPageTitle('Edit Service Subscribed'));
        fetchServiceTypes();
        fetchServiceSubscribed();
    }, []);
    ...
    const fetchServiceSubscribed = async () => {
        if (!id) {
            Swal.fire({ icon: 'error', title: 'Invalid service ID' });
            navigate('/apps/services-subscribed');
            return;
        }
        try {
            const res = await fetch(API_ENDPOINTS.SERVICE.SUBSCRIBED_BY_ID(id));
            const data = await res.json();
            if (data) {
                setServiceSubscribed(data);
            }
        } catch (error) {
            console.error('Error fetching service subscribed:', error);
            Swal.fire({ icon: 'error', title: 'Failed to fetch service subscribed' });
            navigate('/apps/services-subscribed');
        }
    };
    ...
    const saveServiceSubscribed = async () => {
        ...
        try {
            await fetch(API_ENDPOINTS.SERVICE.SUBSCRIBED_BY_ID(id), {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(serviceSubscribed),
            });
            Swal.fire({ icon: 'success', title: 'Service Subscribed updated!' });
            navigate('/apps/services-subscribed');
        } catch (error) {
            console.error(error);
            Swal.fire({ icon: 'error', title: 'Failed to update service subscribed' });
        }
    };

    const deleteServiceSubscribed = async () => {
        if (!id) {
            Swal.fire({ icon: 'error', title: 'Invalid service ID' });
            return;
        }
        ...
        try {
            await fetch(API_ENDPOINTS.SERVICE.SUBSCRIBED_BY_ID(id), { method: 'DELETE' });
    ...
    return (
        <div>
            <div className="flex items-center justify-between flex-wrap gap-4">
                <h2 className="text-xl">Edit Service Subscribed</h2>
            ...
            <div className="mt-5 panel">
                <form onSubmit={e => { e.preventDefault(); saveServiceSubscribed(); }}>
                    ...
                    <div className="flex justify-end items-center mt-8">
                        ...
                        <button type="button" className="btn btn-danger ltr:ml-4 rtl:mr-4" onClick={deleteServiceSubscribed}>
                            Delete
                        </button>
...
export default EditServiceSubscribed; 