...
const Users = () => {
    ...
    const fetchUsers = async () => {
        try {
            ...
            // Filter for staff-level users only
            const staffUsers = users.filter(user => user.level === 'staff');
            setContactList(staffUsers);
            setFilteredItems(staffUsers);
        } catch (error) {
    ...
    const saveUser = async () => {
        ...
        if (!params.id && !params.password) {  // Only require password for new users
                                ...
                                {filteredItems.map((contact: User) => {
                                    return (
                                        <tr key={contact.id}>
                                            ...
                                            <td>
                                                <span className={`badge ${contact.reset_required ? 'badge-outline-warning' : 'badge-outline-success'}`}>
                                                    {contact.reset_required ? 'Yes' : 'No'}
                                                </span>
                                            </td>
                                            <td>
                            ...
                            <div className="bg-white dark:bg-[#1c232f] rounded-md overflow-hidden text-center shadow relative" key={contact.id}>
                                <div className="p-6">
                                    ...
                                    <div className="mt-4 flex flex-col gap-2">
                                        ...
                                        <div>
                                            <span className={`badge ${contact.reset_required ? 'badge-outline-warning' : 'badge-outline-success'}`}>
                                                Reset Required: {contact.reset_required ? 'Yes' : 'No'}
                                            </span>
                                        </div>
                                        <div>
                                            <span className={`badge ${contact.status === 'active' ? 'badge-outline-success' : 'badge-outline-danger'}`}>
                            ...
                            >
                                <Dialog.Panel className="panel border-0 p-0 rounded-lg overflow-hidden w-full max-w-lg text-black dark:text-white-dark">
                                    ...
                                    <div className="text-lg font-medium bg-[#fbfbfb] dark:bg-[#121c2c] ltr:pl-5 rtl:pr-5 py-3 ltr:pr-[50px] rtl:pl-[50px]">
                                        {params.id ? (isEditMode ? 'Edit User' : 'User Details') : 'Add User'}
                                    </div>
                                    <div className="p-5">
                                        {!params.id ? (
                                            <form>
                                                ...
                                                <div className="mb-5">
                                                    <label htmlFor="reset_required">Reset Required</label>
                                                    <input type="checkbox" id="reset_required" className="form-checkbox" checked={params.reset_required} onChange={(e) => setParams({...params, reset_required: e.target.checked})} />
                                                </div>
                                                <div className="mb-5">
                                            ...
                                            <div>
                                                {isEditMode ? (
                                                    <form>
                                                        ...
                                                        <div className="mb-5">
                                                            <label htmlFor="reset_required" className="inline-flex items-center">
                                                                <input type="checkbox" id="reset_required" className="form-checkbox" checked={params.reset_required} onChange={(e) => setParams({...params, reset_required: e.target.checked})} />
                                                                <span className="ltr:ml-2 rtl:mr-2">Reset Required</span>
                                                            </label>
                                                        </div>
                                                        <div className="mb-5">
                                                            <label htmlFor="status">Status</label>
                                                    ...
                                                    <div className="space-y-4">
                                                        ...
                                                        <div className="flex items-center border-b pb-3">
                                                            <div className="w-1/3 font-semibold text-gray-500">Reset Required:</div>
                                                            <div className="w-2/3">
                                                                <span className={`badge ${params.reset_required ? 'badge-outline-warning' : 'badge-outline-success'}`}>
                                                                    {params.reset_required ? 'Yes' : 'No'}
                                                                </span>
                                                            </div>
                                                        </div>
                                                        <div className="flex items-center border-b pb-3">
