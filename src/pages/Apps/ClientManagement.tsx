...
const Clients = () => {
    ...
    const fetchUsers = async () => {
        try {
            ...
            // Filter for client-level users only
            const clientUsers = users.filter(user => user.level === 'client');
            setContactList(clientUsers);
            setFilteredItems(clientUsers);
        } catch (error) {
                            ...
                            >
                                <Dialog.Panel className="panel border-0 p-0 rounded-lg overflow-hidden w-full max-w-lg text-black dark:text-white-dark">
                                    ...
                                    <div className="text-lg font-medium bg-[#fbfbfb] dark:bg-[#121c2c] ltr:pl-5 rtl:pr-5 py-3 ltr:pr-[50px] rtl:pl-[50px]">
                                        {params.id ? (isEditMode ? 'Edit Client' : 'Client Details') : 'Add Client'}
                                    </div>
                                    <div className="p-5">
                                        {!params.id ? (
                                            <form>
